    ...
    with gr.Blocks(analytics_enabled=False) as openpose_editor:
        with gr.Row():
            with gr.Column():
                ...
                with gr.Row():
                    ...
                    json_input = gr.UploadButton(
                        label="Load from JSON",
                        file_types=[".json"],
                        elem_id="openpose_json_button",
                    )
                    png_input = gr.UploadButton(
                        label="Detect from Image",
                        file_types=["image"],
                        type="bytes",
                        elem_id="openpose_detect_button",
                    )
                    bg_input = gr.UploadButton(
                        label="Add Background Image",
                        file_types=["image"],
                        elem_id="openpose_bg_button",
                    )
                with gr.Row():
            ...
            with gr.Column():
                # GRADIOOOOO...
                canvas = gr.HTML(
                    '<canvas id="openpose_editor_canvas" width="512" height="512" style="margin: 0.25rem; border-radius: 0.25rem; border: 0.5px solid"></canvas>'
                )
        ...
        def estimate(file):
            ...
            if body_estimation is None:
                model_path = os.path.join(
                    models_path, "openpose", "body_pose_model.pth"
                )
                if not os.path.isfile(model_path):
                    body_model_path = "https://huggingface.co/lllyasviel/ControlNet/resolve/main/annotator/ckpts/body_pose_model.pth"
                    load_file_from_url(
                        body_model_path, model_dir=os.path.join(models_path, "openpose")
                    )
        ...
        def savePreset(name, data):
            if name:
                ...
                return gr.update(
                    choices=sorted(presets.keys()), value=name
                ), json.dumps(data)
        ...
        bg_input.upload(
            None, [], [width, height], _js="() => {addBackground('openpose_bg_button')}"
        )
        ...
        png_input.upload(
            ...
            [],
            [width, height],
            _js="() => {addBackground('openpose_detect_button')}",
        )
        add.click(None, [], None, _js="addPose")
        send_t2t.click(
            None, select_target_index, None, _js="(i) => {sendImage('txt2img', i)}"
        )
        send_i2i.click(
            None, select_target_index, None, _js="(i) => {sendImage('img2img', i)}"
        )
        ...
        json_input.upload(
            ...
            json_input,
            [width, height],
            _js="() => {loadJSON('openpose_json_button')}",
        )
        ...
        preset_list.change(
            lambda selected: json.dumps(presets[selected]), preset_list, preset
        )
